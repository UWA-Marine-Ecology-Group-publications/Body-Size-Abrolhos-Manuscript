...
data2 <- data2 %>% mutate(errorbar= average + se)

Large<-data2%>%
   filter(Percentage.of.Sites>=0.20)

xx = ggplot(Large, aes(x = Genus.Species, y = Status)) + 
  geom_point(aes(size = average, fill = Status), alpha = 0.75, shape = 21) + 
  geom_point(aes(size = errorbar,colour = Status), alpha = 0.75, shape = 3)+
...
data3 <- data3 %>% mutate(errorbar= average + se)
data3 <- data3 %>% mutate(errorbar2= average - se)
...
data4 <- data4 %>% mutate(errorbar= average + se)
